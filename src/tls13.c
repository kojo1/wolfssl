...
static int RestartHandshakeHashWithCookie(WOLFSSL* ssl, Cookie* cookie)
{
    ...
    if (cookieDataSz > hashSz + OPAQUE16_LEN + OPAQUE8_LEN) {
        keyShareExt = 1;
...
static int SendTls13CertificateVerify(WOLFSSL* ssl)
{
    ...
    /* can be negative */
    if (ssl->options.dtls)
        record_layer_hdr_extra = Dtls13GetRlHeaderLength(1) - RECORD_HEADER_SZ;
    else
...
static int SendTls13Finished(WOLFSSL* ssl)
{
    ...
    if (ssl->options.side == WOLFSSL_SERVER_END) {
        ...
        if (isDtls) {
            ...
            ssl->dtls13PeerEpoch = DTLS13_EPOCH_TRAFFIC0;
